...
void CFluidSolver::velocity_advection()
{
	for (int i = 1; i < n-1; i++)
		for (int j=1; j < n-1; j++) {
			...
			//Get previous point x and y (point - velocity*h)
			double it = i - velocity[i+j*n].x*h;
			double jt = j - velocity[i+j*n].y*h;

			if(it < .5) it = .5;
			if(jt < .5) jt = .5;
			if(it > n-1.5) it = n-1.5;
			if(jt > n-1.5) jt = n-1.5;

			//Find floor and ceil of 
			int floorX = (int)floor(it);
			int floorY = (int)floor(jt);
			int ceilX = (int)ceil(it);
			int ceilY = (int)ceil(jt);

			double brd = (it-floorX)*(jt-floorY);
			double bld = (ceilX-it)*(jt-floorY);
			double trd = (it-floorX)*(ceilY-jt);
			double tld = (ceilX-it)*(ceilY-jt);

			advected_velocity[i+j*n] = ((velocity[floorX+floorY*n]*tld) + (velocity[ceilX+floorY*n]*trd) + (velocity[floorX+ceilY*n]*bld) + (velocity[ceilX+ceilY*n]*brd));
		}
}