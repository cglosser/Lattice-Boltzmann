...
class Lattice {
 public:
   ...
   * indices */
  Lattice(const int, const int, const int threadCount);
  ...
  */
  ...
  /**
  * @brief Set density at a certain point on the grid
  */
  void SetDensity(int site, double newDensity);
  ...
  */
  ...
  /**
  * @brief Set the last update time so we have an appropriate timestep the first time we update
  */
 ...
 private:
  ...
  bool done = false;
  vector<int> updateStates; //0 = idle, 1 = streamingUpdate, 2 = collisionUpdate
  ...
  std::chrono::time_point<std::chrono::system_clock> lastUpdateTime, currentUpdateTime;
   ...
   * bouncebacks.
   */
  void streamingUpdate(int start, int end);

  void postStreamingUpdate();
   ...
   */
  void collisionUpdate(int start, int end);

  void postCollisionUpdate();

  void WorkerThread(int homeAddress, int start, int end);
};
